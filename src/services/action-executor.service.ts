  ...
  static async execute(action: ResolvedAction, userId: string): Promise<ExecutionResult> {
    try {
      switch (action.intent) {
        ...
        case 'QUERY':
          return executeQuery(action.data as QueryPayload, userId);
...
async function executeQuery(payload: QueryPayload, userId: string): Promise<ExecutionResult> {
  ...
  switch (payload.query_type) {
    ...
    case 'ACCOUNT_STATEMENT': {
      ...
      if (txs.length === 0) {
        ...
        msg += txs.map((t) => formatTransaction(t)).join('\n');
      }
    ...
    case 'LIST_TRANSACTIONS': {
      ...
      const header = buildRangeHeader(f.date_from, f.date_to);
      return { ok: true, message: `📋 Transacciones${header}:\n${txs.map(formatTransaction).join('\n')}` };
    }
...
function buildRangeHeader(dateFrom?: string, dateTo?: string): string {
