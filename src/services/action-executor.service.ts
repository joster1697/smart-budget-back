...
export class ActionExecutorService {
      ...
      switch (action.intent) {
        case "CREATE_TRANSACTION": {
          ...
          if (!accountId) {
            return {
              ok: false,
              message:
                "No se pudo determinar la cuenta. Por favor indica a cuál cuenta registrar la transacción.",
            };
          }
          ...
          const categoryId =
            ...
            (d.account_id && action.resolved_id
              ? action.resolved_id
              : undefined);
          ...
          const convertedMsg = isConverted ? ` (convertido a ₡${Number(createdTx.amount).toLocaleString()})` : "";
          ...
          let balanceNote = "";
          if (isCredit) {
            balanceNote = ` | Deuda en ${account.name}: ₡${Math.abs(newBalance).toLocaleString()}`;
          } else if (isNegative) {
            balanceNote = ` | ⚠️ ${account.name} quedó con saldo negativo: ₡${newBalance.toLocaleString()}`;
          } else {
            balanceNote = ` | Saldo en ${account.name}: ₡${newBalance.toLocaleString()}`;
          }
        ...
        case "UPDATE_TRANSACTION": {
          ...
          if (!action.resolved_id)
            return {
              ok: false,
              message: "No se especificó qué transacción actualizar.",
            };
          const changes = {
            ...d.changes,
            date: d.changes.date ? new Date(d.changes.date) : undefined,
          };
        ...
        case "DELETE_TRANSACTION": {
          if (!action.resolved_id)
            return {
              ok: false,
              message: "No se especificó qué transacción eliminar.",
            };
        ...
        case "UPDATE_ACCOUNT": {
          ...
          if (!action.resolved_id)
            return {
              ok: false,
              message: "No se especificó qué cuenta actualizar.",
            };
        ...
        case "UPDATE_CATEGORY": {
          ...
          if (!action.resolved_id)
            return {
              ok: false,
              message: "No se especificó qué categoría actualizar.",
            };
          await CategoryService.updateCategory(action.resolved_id, userId, {
            name: d.changes.name,
            user_id: userId,
          });
          return {
            ok: true,
            message: `✅ Categoría renombrada a "${d.changes.name}".`,
          };
        }
        ...
        case "DELETE_CATEGORY": {
          if (!action.resolved_id)
            return {
              ok: false,
              message: "No se especificó qué categoría eliminar.",
            };
  ...
  private static async executeQuery(
    ...
    let resolvedAccount:
      | Awaited<ReturnType<typeof AccountService.getAccountsByUserId>>[number]
      | undefined;
    ...
    switch (payload.query_type) {
      case "LIST_ACCOUNTS": {
        ...
        if (accounts.length === 0)
          return { ok: true, message: "No tienes cuentas registradas aún." };
      ...
      case "LIST_CATEGORIES": {
        ...
        if (categories.length === 0)
          return { ok: true, message: "No tienes categorías registradas aún." };
      ...
      case "ACCOUNT_STATEMENT": {
        if (!resolvedAccountId || !resolvedAccount) {
          return {
            ok: false,
            message: `No encontré una cuenta con el nombre "${f.account_name ?? "(no especificado)"}". Verifica el nombre e inténtalo de nuevo.`,
          };
        }
        ...
        if (txs.length === 0) {
          ...
          msg += txs
            .map((t) => ActionExecutorService.formatTransaction(t))
            .join("\n");
        }
      ...
      case "LIST_TRANSACTIONS": {
        ...
        if (txs.length === 0)
          return {
            ok: true,
            message: "No encontré transacciones con esos criterios.",
          };
        const header = ActionExecutorService.buildRangeHeader(
          f.date_from,
          f.date_to,
        );
        return {
          ok: true,
          message: `📋 Transacciones${header}:\n${txs.map((t) => ActionExecutorService.formatTransaction(t)).join("\n")}`,
        };
      }
      ...
      case "SPENDING_SUMMARY": {
        ...
        if (txs.length === 0)
          return {
            ok: true,
            message: "No encontré transacciones para ese período.",
          };
